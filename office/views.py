...
def office_index(request):
    ...
    context = {
        ...
        "total_teachers": core_models.Teacher.objects.filter(is_active=True).count(),
...
def admin_teachers(request):
    ...
    if is_admin_mode:
        ...
        raw_teachers = core_models.Teacher.objects.filter(is_active=True).order_by('-id')
    ...
    for t in raw_teachers:
        ...
        students_enrolled = core_models.Student.objects.filter(course__in=my_courses)
...
def admin_classroom(request):
    ...
    if request.method == "POST":
        ...
        teacher = core_models.Teacher.objects.filter(pk=teacher_id, is_active=True).first() if teacher_id else None
    ...
    all_teachers = core_models.Teacher.objects.filter(is_active=True)
...
def admin_students(request):
    ...
    return render(request, 'office/admin_students.html', {"students": paged_students, "is_admin_mode": is_admin_mode})
